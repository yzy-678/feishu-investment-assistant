...
- 实时行情、技术分析、行业属性由程序直接渲染，你只输出“📌 核心结论”“📌 关键信号”“⚠ 风险”。
...
class MarketAgent(BaseAgent):
    ...
    def _format_rating_block(rating: InvestmentRating) -> str:
        ...
        return "\n".join([
            "📊 AI评分",
            f"综合：{rating.rating_level.value}｜{rating.total_score:.0f}/100｜{change_text}",
            (
                f"分项：趋势{rating.trend_score:.0f}｜量价{rating.volume_score:.0f}｜"
                f"K线{rating.breakout_score:.0f}｜强度{rating.strength_score:.0f}"
            ),
            f"板块：{sector_status}｜昨日：{previous_score}",
        ])
    ...
    def _build_stock_data_card(
        ...
        return "\n".join([
            "📊 个股卡片",
            f"{name}（{stock.symbol}）｜{industry_text}",
            (
                f"价格：{self._format_price(self._quote_value(quote, 'price'))}｜"
                f"涨跌幅：{self._format_pct(self._quote_value(quote, 'change_pct'))}｜"
                f"成交额：{self._format_amount(self._quote_value(quote, 'amount'))}"
                if quote_valid
                else "价格：实时行情暂缺｜涨跌幅：实时行情暂缺｜成交额：实时行情暂缺"
            ),
            f"概念：{self._truncate_inline(concepts_text, 42)}",
            (
                f"技术：MA5 {ma5}｜MA20 {ma20}｜MACD {macd}"
                if technical_valid
                else "技术：技术指标暂缺"
            ),
            f"来源：{self._quote_value(quote, 'source') if quote_valid else '实时行情暂缺'}｜{self._quote_value(quote, 'timestamp') if quote_valid else '时间暂缺'}",
        ])
    ...
    def _build_stock_reply_format_rules() -> str:
        return "\n".join([
            "回复格式要求：",
            "- 程序已直接渲染“📊 个股卡片”和“📊 AI评分”。",
            "- 默认总输出必须在600字以内，适合飞书一屏阅读。",
            "- 你只输出：📌 核心结论、📌 关键信号、⚠ 风险。",
            "- 核心结论只写1段，关键信号最多3条，风险最多2条。",
            "- 禁止长篇研报、行业宏观叙事、重复解释评分逻辑。",
        ])
    ...
    def _build_response_length_rules(cls, message: str = "") -> str:
        ...
        return "\n".join([
            "回复长度要求：默认总输出不超过600字。",
            "- 输出结构固定为：📊 个股卡片、📊 AI评分、📌 核心结论、📌 关键信号、⚠ 风险。",
            "- 关键信号最多3条，风险最多2条，每条只写1行。",
            "- 删除重复解释，不写长篇研报式内容。",
        ])
    ...
    def _compose_final_response(
        ...
        if quote_block.strip().startswith(("📈 实时行情", "【数据卡片】", "📊 个股卡片")):
            return sanitize_text(
                self._compose_stock_card_response(
                    quote_block.strip(),
                    analysis,
                    response_style,
                )
    ...
    @classmethod
    def _compose_stock_card_response(
        cls,
        quote_block: str,
        analysis: str,
        response_style: str = "normal",
        ...
        response = "\n\n".join([
            quote_block,
            cls._ensure_stock_ai_sections(analysis, response_style),
        ])
        return cls._enforce_stock_response_limit(response)
    ...
    def _ensure_stock_ai_sections(analysis: str, response_style: str = "normal") -> str:
        ...
        conclusion, signals, risks = MarketAgent._extract_card_sections(text)
        ...
        conclusion_text = MarketAgent._truncate_inline(
            conclusion or "当前信号以程序评分为准，适合继续观察。",
            95,
        )
        signal_lines = [
            f"{index}. {MarketAgent._truncate_inline(item, 52)}"
    ...
    def _limit_analysis_text(analysis: str, response_style: str) -> str:
        text = str(analysis or "").strip()
        limit = 160 if response_style == "brief" else 520
        if response_style == "detailed" or len(text) <= limit:
            return text
        return text[:limit].rstrip() + "..."

    @staticmethod
    def _extract_card_sections(analysis: str) -> tuple[str, list[str], list[str]]:
        ...
        for raw_line in text.splitlines():
            ...
            if any(marker in normalized for marker in ("核心结论", "AI综合判断")):
                current = "conclusion"
                tail = MarketAgent._strip_section_heading(line)
                if tail:
                    buckets[current].append(tail)
                continue
            if any(marker in normalized for marker in ("关键信号", "3条逻辑", "逻辑")):
                current = "signals"
                tail = MarketAgent._strip_section_heading(line)
                if tail:
                    buckets[current].append(tail)
            ...
            if "风险" in normalized:
                ...
                if tail:
                    buckets[current].append(tail)
                continue
            buckets[current].append(MarketAgent._clean_bullet(line))

        conclusion = " ".join(buckets["conclusion"]).strip()
        if not conclusion:
            conclusion = MarketAgent._first_sentence(" ".join(buckets["other"]))

        signals = buckets["signals"] or [
            item for item in buckets["other"] if not MarketAgent._looks_like_risk(item)
        ]
        risks = buckets["risks"] or [
            item for item in buckets["other"] if MarketAgent._looks_like_risk(item)
        ]
        return (
            MarketAgent._clean_bullet(conclusion),
            MarketAgent._dedupe_lines(signals, limit=3),
            MarketAgent._dedupe_lines(risks, limit=2),
        )

    @staticmethod
    def _strip_section_heading(line: str) -> str:
        text = re.sub(r"^[#\s【】\[\]📌⚠🧠]+", "", str(line or "")).strip()
        text = re.sub(
            r"^(AI综合判断|风险提示|核心结论|关键信号|3条逻辑|3条风险|逻辑|风险)\s*[:：]?",
            "",
            text,
        ).strip()
        return text
    ...
    def _clean_bullet(line: str) -> str:
        return re.sub(r"^[\-*•\d.、）)\s]+", "", str(line or "").strip()).strip()
    ...
    def _looks_like_risk(line: str) -> bool:
        return any(
            marker in str(line or "")
            for marker in ("风险", "回落", "跌破", "放量下跌", "退潮", "不构成", "缺失", "未接入")
        )
    ...
    def _dedupe_lines(lines: list[str], limit: int) -> list[str]:
        result: list[str] = []
        for line in lines:
            clean = MarketAgent._clean_bullet(line)
    ...
    @staticmethod
    def _enforce_stock_response_limit(response: str, limit: int = 600) -> str:
        text = sanitize_text(response).strip()
        if len(text) <= limit:
            return text
        ...
        for line in lines:
            ...
            if allowance <= 0:
                break
            next_line = line if len(line) <= allowance else line[: max(0, allowance - 1)] + "…"
            trimmed.append(next_line)
